...
# ============================================================
# VERTEX - ADAPTIVE GRID EQUIPMENT WORKSPACE V7R3
#
...
$backup=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R3_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R3_FAILED.$stamp"
$report=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R3.$stamp.json"
...
function EnsureBootstrapBase {
  ...
  if(-not(Test-Path -LiteralPath $editorShim)){
    ...
    if($editorSource){
      ...
      $relative=[IO.Path]::GetRelativePath($shimDir,$editorSource.FullName).Replace('\','/')
      if(-not $relative.StartsWith('.')){$relative="./$relative"}
      $relative=$relative -replace '\.(ts|tsx)$',''

      WriteUtf8 $editorShim "export * from '$relative'`n"
      Write-Host "    Found editor API : $($editorSource.FullName)" -ForegroundColor Green
      Write-Host "    Adapter shim     : $editorShim" -ForegroundColor Green
    }
    else {
...
function unbound(name: string): never {
  throw new Error(
    ...
    'V7R3 refused to invent Tauri command names.'
  )
}
...
'@
    ...
    Write-Host "    vertex-editor.ts already exists: PRESERVED" -ForegroundColor Green
  }
