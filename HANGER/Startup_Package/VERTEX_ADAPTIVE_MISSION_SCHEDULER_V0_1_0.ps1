...
<#
VERTEX — ADAPTIVE MISSION SCHEDULER V0.1.0
...
function Group-IntoWaves {
    ...
    if ($PreferResidency) {
        ...
        $working = @($working | Sort-Object priority -Descending)
    }
...
Banner 'VERTEX — ADAPTIVE MISSION SCHEDULER V0.1.0'
...
$queue = [ordered]@{
    schema='vertex.scheduler.mission-queue.v1'
    schedule_id=$scheduleId
    formation_id=$formationId
    mission_class=$missionClass
    created_at=(Get-Date).ToString('o')
    queue=@($tasks | Sort-Object priority -Descending)
}
...
$plan = [ordered]@{
    schema='vertex.scheduler.adaptive-mission-plan.v1'
    schedule_id=$scheduleId
    formation_id=$formationId
    mission_class=$missionClass
...
$busHandoff = [ordered]@{
    schema='vertex.load-balancing-bus.handoff.v1'
    schedule_id=$scheduleId
    mission_class=$missionClass
...
Write-Json $receiptPath ([ordered]@{
    schema='vertex.scheduler.plan-receipt.v1'
...
foreach ($t in ($tasks | Sort-Object priority -Descending)) {
    Write-Host (
