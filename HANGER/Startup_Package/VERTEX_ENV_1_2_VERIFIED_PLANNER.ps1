...
Write-Host ' VERTEX ENV-1 - ENVIRONMENT PLANNER V1.2' -ForegroundColor Magenta
...
Write-Host ' VERIFIED SHELL -> ENV-0 V4.1 TERMINAL EVIDENCE' -ForegroundColor Magenta
Write-Host '============================================================' -ForegroundColor Magenta
...
$terminal = P $p windows_terminal
$terminalFound = [bool](P $terminal found $false)
$terminalDefaultPs7 = [bool](P (P $p capability_signals) windows_terminal_default_ps7 $false)
$terminalVertexStart = [bool](P (P $p capability_signals) windows_terminal_vertex_start $false)

if ($terminalFound -and $terminalDefaultPs7 -and $terminalVertexStart) {
    $find.Add((F shell GREEN 'Windows Terminal profile verified' (
        "Default profile: {0} / Start: {1}" -f
        (P $terminal default_profile_name 'UNKNOWN'),
        (P $terminal starting_directory 'UNKNOWN')
    )))
}
elseif ($terminalFound) {
    $find.Add((F shell YELLOW 'Windows Terminal profile incomplete' (
        "Default profile: {0} / PS7: {1} / Vertex start: {2}" -f
        (P $terminal default_profile_name 'UNKNOWN'),
        $terminalDefaultPs7,
        $terminalVertexStart
    ) 'Verify defaultProfile and startingDirectory, then re-run ENV-0 V4.1.'))
}
else {
    $find.Add((F shell UNKNOWN 'Windows Terminal not machine-verified' 'ENV-0 did not capture a usable Windows Terminal profile.' 'Run ENV-0 V4.1 or later.'))
}
...
$storagePlacementStatus = if ($best) { $best.status } else { 'RED' }
...
$portOwners = @()
foreach ($port in @(3000,5173,5432,8000,8080,11434)) {
    if (@($lis | Where-Object { [int](P $_ LocalPort 0) -eq $port }).Count -eq 0) { continue }
    $owners = @(Get-PortOwner $port)
    $portOwners += $owners
    $names = @($owners | ForEach-Object { $_.process } | Sort-Object -Unique)
    $ownerText = if ($names.Count) { $names -join ', ' } else { 'UNKNOWN' }
    if ($port -eq 11434 -and $names -contains 'ollama') {
        $find.Add((F ports GREEN 'Ollama port 11434 expected' "Owner: $ownerText" 'No action required.'))
    } else {
        $find.Add((F ports YELLOW "Port $port is listening" "Owner: $ownerText" 'Resolve ownership before binding.'))
    }
}
...
$overall='GREEN';$max=0
$hostFindings = @($find | Where-Object { $_.area -ne 'storage' })
foreach ($x in $hostFindings){$r=Rank $x.status;if ($r-gt$max){$max=$r;$overall=$x.status}}
$roleStatus='GREEN';$roleMax=0
foreach ($ri in $roles){$rr=Rank $ri.status;if ($rr-gt$roleMax){$roleMax=$rr;$roleStatus=$ri.status}}
...
$plan=[ordered]@{schema='vertex.environment.plan.v1.2';mission_id=$Mission;generated_at=(Get-Date).ToString('o');source_profile=$pf.FullName;mode='READ_ONLY_PLANNING';host_status=$overall;role_status=$roleStatus;storage_placement_status=$storagePlacementStatus;shell_verified=($terminalFound -and $terminalDefaultPs7 -and $terminalVertexStart);recommended_volume=$best;placement_candidates=@($cand|sort score -Descending);port_owners=@($portOwners);role_matrix=$roles;findings=@($find);execution_policy=[ordered]@{install=$false;uninstall=$false;registry_mutation=$false;service_mutation=$false;firewall_mutation=$false;path_mutation=$false;requires_human_gate_before_env2=$true};next_mission='ENV-2 Package Lifecycle / Deployment Executor'}
...
============================================================
 ...
 Storage Placement  : $storagePlacementStatus
 Shell Verified     : $(if($terminalFound -and $terminalDefaultPs7 -and $terminalVertexStart){'GREEN'}else{'ATTENTION'})
