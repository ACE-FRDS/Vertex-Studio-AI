...
# ============================================================
# VERTEX CIC — VERTEXHUB HOT INSTALL / PACKAGE BROWSER V2
...
$backup=Join-Path $reports "VERTEX_HUB_HOT_INSTALL_BROWSER_V2_BACKUP.$stamp"
$failed=Join-Path $reports "VERTEX_HUB_HOT_INSTALL_BROWSER_V2_FAILED.$stamp"
$report=Join-Path $reports "VERTEX_HUB_HOT_INSTALL_BROWSER_V2.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX — VERTEXHUB HOT INSTALL / PACKAGE BROWSER V2
...
'@
  ...
  # V2: explicit Tauri JSON serialization dependency.
  # V1 proved that vsa-shell-desktop did not own serde_json directly.
  $tauriCargoText=[IO.File]::ReadAllText($tauriCargo)
  if($tauriCargoText -notmatch '(?m)^\s*serde_json\s*='){
    $dependencyAnchor=[regex]::Match($tauriCargoText,'(?m)^\[dependencies\]\s*$')
    if(-not $dependencyAnchor.Success){
      throw 'Tauri [dependencies] anchor missing; refusing dependency mutation.'
    }

    $dependencyInsert=$dependencyAnchor.Index+$dependencyAnchor.Length
    $tauriCargoText=$tauriCargoText.Insert(
      $dependencyInsert,
      "`r`nserde_json = `"1`""
    )
...
'@
  ...
  $audits=@(
    ...
    [pscustomobject]@{Name='Tauri serde_json dependency';Pass=([IO.File]::ReadAllText($tauriCargo) -match '(?m)^\s*serde_json\s*=')},
  ...
  [ordered]@{
    schema='vertex.cic.vertex-hub-hot-install-package-browser.v2'
  ...
  Write-Host @"
============================================================
 VERTEX — HOT INSTALL / PACKAGE BROWSER V2 GREEN
============================================================
...
"@ -ForegroundColor Green
  ...
  Write-Host ' VERTEXHUB HOT INSTALL / PACKAGE BROWSER V2 RED — DAMAGE CONTROL' -ForegroundColor Red
