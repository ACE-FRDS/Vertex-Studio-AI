...
<#
VERTEX ENV-2 V2.4.10 — VERSION LINEAGE NORMALIZER
...
Write-Host ' VERTEX ENV-2 V2.4.10 — VERSION LINEAGE NORMALIZER' -ForegroundColor Magenta
...
foreach ($c in $candidates) {
    ...
    if ($best) {
        ...
        $newTexts = @(
            [string]$best.path,
            [string]$best.file_identity.file_version,
            [string]$best.file_identity.product_version,
            [string]$best.app_identity.display_version,
            [string]$best.app_identity.display_name
        )
...
$report = [ordered]@{
    ...
    mission       = 'VERTEX_ENV_2_V2_4_10_VERSION_LINEAGE_NORMALIZER'
