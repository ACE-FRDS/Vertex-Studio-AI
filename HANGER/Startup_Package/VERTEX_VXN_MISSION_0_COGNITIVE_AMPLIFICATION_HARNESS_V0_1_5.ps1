...
<#
VERTEX VXN — MISSION 0 COGNITIVE AMPLIFICATION HARNESS V0.1.5
...
Banner 'VERTEX VXN — MISSION 0 COGNITIVE AMPLIFICATION HARNESS V0.1.5'
...
foreach ($variant in $variants) {
    ...
    try {
        $normalized = Normalize-ModelJson -Text $call.Text
    ...
    Write-Host "      model=OK latency=$($call.LatencyMs)ms json=$jsonValid shape=$jsonShape normalize=$normalizationStrategy schema=$([math]::Round($schemaScore,2)) lock=$lockScore scope=$scopeScore authority=$authorityScore missing=$($missingFields.Count) $evalState"
...
$receipt = [ordered]@{
    schema='vertex.vxn.mission0.receipt.v1.5'
    run_id=$runId
    timestamp=(Get-Date).ToString('o')
    provider=$runtime.Provider
    model=$runtime.Model
...
Banner 'VXN MISSION 0 V0.1.5 — FIRST SIGNAL COMPLETE'
Write-Host "Provider : $($runtime.Provider)"
Write-Host "Model    : $($runtime.Model)"
