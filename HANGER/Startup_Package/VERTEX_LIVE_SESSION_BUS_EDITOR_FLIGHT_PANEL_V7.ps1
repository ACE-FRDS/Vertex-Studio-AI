...
# ============================================================
# VERTEX CIC — LIVE SESSION BUS / EDITOR FLIGHT PANEL V7
...
$backup=Join-Path $reports "LIVE_SESSION_BUS_V7_BACKUP.$stamp"
$failed=Join-Path $reports "LIVE_SESSION_BUS_V7_FAILED.$stamp"
$report=Join-Path $reports "LIVE_SESSION_BUS_V7.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX — LIVE SESSION BUS / EDITOR FLIGHT PANEL V7
...
foreach($requiredField in @('execution_id','mission_id','agent_id')){
...
fn vertex_live_publish_preflight(session: &FleetControllerSession) {
    ...
    let (dispatch_id, executions) = match session.current_wave.dispatch.as_ref() {
        Some(dispatch) => {
            let executions = dispatch.$assignmentField
                ...
                .map(|execution| {
                    format!(
                        "{{\"execution_id\":{},\"mission_id\":{},\"agent_id\":{}}}",
                        vertex_live_quote(&execution.execution_id),
                        vertex_live_quote(&execution.mission_id),
    ...
    let json = format!(
        "{{\"schema\":\"vertex.mothership.live-session.v1\",\"kind\":\"wave_scheduled\",\"timestamp_ms\":{},\"session\":{{\"session_id\":{},\"status\":{},\"completed_waves\":{}}},\"wave\":{{\"wave_id\":{},\"ready\":{},\"blocked\":{},\"waiting\":{}}},\"dispatch\":{{\"dispatch_id\":{},\"executions\":{}}},\"genesis\":{{\"event_count\":0}},\"vsp\":null}}",
        now_ms,
    ...
    <div v-if="expanded" class="detail">
      <section>
        ...
        >
          ...
          <small>{{ execution.mission_id || '-' }}</small>
        </article>
      </section>
...
'@
  ...
  [ordered]@{
    schema='vertex.cic.live-session-bus-editor-flight-panel.v7'
    ...
    surfaces=@{
      ...
      wave='LIVE'
      dispatch='LIVE'
      execution='LIVE_SCHEDULED_AND_PROCESS_RESULT'
  ...
  Write-Host @"
============================================================
 VERTEX — LIVE SESSION BUS V7 GREEN
============================================================
...
"@ -ForegroundColor Green
  ...
  Write-Host ' LIVE SESSION BUS V7 RED — DAMAGE CONTROL' -ForegroundColor Red
