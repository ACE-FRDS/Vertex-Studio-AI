...
<#
VERTEX ENV-2 V2.6.1 — TRANSACTION FIREWALL EXECUTOR
...
function Get-VertexPrivilegeState {
    $isAdmin = Test-VertexAdministrator

    $netSecurityAvailable = $false
    try {
        $netSecurityAvailable = $null -ne (Get-Command Get-NetFirewallRule -ErrorAction Stop)
    }
    catch {}

    $persistentStoreReadable = $false
    try {
        $null = Get-NetFirewallRule -PolicyStore PersistentStore -ErrorAction Stop | Select-Object -First 1
        $persistentStoreReadable = $true
    }
    catch {}

    # Conservative write capability inference:
    # actual write is NOT tested here because preflight must remain non-mutating.
    $persistentStoreWriteLikely = ($isAdmin -and $netSecurityAvailable -and $persistentStoreReadable)

    return [pscustomobject][ordered]@{
        is_administrator = $isAdmin
        netsecurity_available = $netSecurityAvailable
        persistent_store_readable = $persistentStoreReadable
        persistent_store_write_likely = $persistentStoreWriteLikely
    }
}

Write-Host '============================================================' -ForegroundColor Magenta
Write-Host ' VERTEX ENV-2 V2.6.1 — TRANSACTION FIREWALL EXECUTOR' -ForegroundColor Magenta
...
$privilege = Get-VertexPrivilegeState

Write-Host ''
Write-Host '[0/5] PRIVILEGE PRE-FLIGHT' -ForegroundColor Cyan
Write-Host "  Administrator             : $($privilege.is_administrator)"
Write-Host "  NetSecurity available     : $($privilege.netsecurity_available)"
Write-Host "  PersistentStore readable  : $($privilege.persistent_store_readable)"
Write-Host "  Write capability likely   : $($privilege.persistent_store_write_likely)"

if (-not $privilege.netsecurity_available) {
    throw 'PRE_FLIGHT_DENIED_PRIVILEGE: NetSecurity cmdlets are unavailable.'
}

if (-not $privilege.persistent_store_readable) {
    throw 'PRE_FLIGHT_DENIED_PRIVILEGE: PersistentStore cannot be read.'
}

if ($Mode -eq 'Execute' -and -not $privilege.is_administrator) {
    throw 'PRE_FLIGHT_DENIED_PRIVILEGE: Execute mode requires an elevated Administrator PowerShell session.'
}

if ($Mode -eq 'Execute' -and -not $privilege.persistent_store_write_likely) {
    throw 'PRE_FLIGHT_DENIED_PRIVILEGE: Firewall PersistentStore write capability is not established.'
}
...
if ($Mode -eq 'DryRun') {
    ...
    Write-Host ' V2.6.1 DRY RUN : GREEN' -ForegroundColor Green
...
if ($commitGreen) {
    Write-Host ''
    Write-Host '============================================================' -ForegroundColor Green
    Write-Host ' V2.6.1 TRANSACTION EXECUTION : COMMIT_GREEN' -ForegroundColor Green
    Write-Host " Candidate : $CandidateId"
