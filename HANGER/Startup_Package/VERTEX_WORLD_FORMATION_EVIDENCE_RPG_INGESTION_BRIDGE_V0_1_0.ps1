...
<#
VERTEX WORLD — FORMATION EVIDENCE RPG INGESTION BRIDGE V0.1.0
...
Banner 'VERTEX WORLD — FORMATION EVIDENCE RPG INGESTION BRIDGE V0.1.0'
...
foreach ($file in $receiptFiles) {
    ...
    if (Test-Path -LiteralPath $battlePath) {
        Write-Host "  KEEP : $runId already ingested."
...
Write-Json $ingestionIndexPath ([ordered]@{
    schema='vertex.world.rpg.formation-evidence-ingestion.v1'
...
Write-Json $bridgeReceiptPath ([ordered]@{
    schema='vertex.world.rpg.formation-evidence-ingestion-receipt.v1'
...
Write-Host ''
Write-Host 'VERTEX WORLD — LEARNING LOOP CLOSED.'
