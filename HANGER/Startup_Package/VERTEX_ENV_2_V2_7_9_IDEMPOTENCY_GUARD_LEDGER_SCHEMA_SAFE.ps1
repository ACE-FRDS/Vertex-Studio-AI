...
Write-Host ' VERTEX ENV-2 V2.7.9 — IDEMPOTENCY GUARD' -ForegroundColor Magenta
...
if (Test-Path -LiteralPath $LedgerPath -PathType Leaf) {
    ...
    foreach ($entry in $entries) {
        ...
        $entryLifecycle = [string](Get-SafeProperty $entry 'lifecycle_state' '')
...
foreach ($file in $postReports) {
    try {
        ...
        $rl = [string](Get-SafeProperty $r 'lifecycle_state' '')
        if (-not $rl) {
            $rl = [string](Get-SafeProperty $r 'transaction_lifecycle' '')
        }
        if (-not $rl) {
            $rl = [string](Get-SafeProperty $r 'lifecycle' '')
        }
        ...
        $verificationStatus = [string](Get-SafeProperty $r 'verification_status' '')

        if (($candidateMatch -or $txnMatch) -and (
            $rs -eq 'COMMITTED_VERIFIED' -or
            $rl -eq 'COMMITTED_VERIFIED' -or
            $verificationStatus -eq 'POST_COMMIT_GREEN'
        )) {
            $committedMatches.Add([pscustomobject]@{
                candidate_id = $rc
                transaction_id = $rt
...
@(
    '============================================================',
    ' VERTEX IDEMPOTENCY GUARD V2.7.9',
    '============================================================',
...
Write-Host " V2.7.9 IDEMPOTENCY GUARD : $decision" -ForegroundColor $color
