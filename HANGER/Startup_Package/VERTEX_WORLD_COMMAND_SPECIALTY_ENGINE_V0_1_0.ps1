...
<#
VERTEX WORLD — COMMAND SPECIALTY ENGINE V0.1.0
...
Banner 'VERTEX WORLD — COMMAND SPECIALTY ENGINE V0.1.0'
...
foreach ($missionClass in ($byMission.Keys | Sort-Object)) {
    ...
    $missionIndex += [ordered]@{
        mission_class=$missionClass
        best_command_model=[string](Get-Prop $best 'model' '')
        best_specialty_score=[double](Get-Prop (Get-Prop $best 'specialty' $null) 'conservative_score' 0)
        confidence=[string](Get-Prop (Get-Prop $best 'confidence' $null) 'tier' 'PROVISIONAL')
        candidates=$entries
    }
}

$output = [ordered]@{
    schema='vertex.world.rpg.command-specialty.v1'
...
Write-Json $receiptPath ([ordered]@{
    schema='vertex.world.rpg.command-specialty-receipt.v1'
