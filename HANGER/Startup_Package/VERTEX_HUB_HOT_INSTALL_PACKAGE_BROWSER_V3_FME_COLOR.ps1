...
# ============================================================
# VERTEX CIC — VERTEXHUB HOT INSTALL / PACKAGE BROWSER V3
...
$backup=Join-Path $reports "VERTEX_HUB_HOT_INSTALL_BROWSER_V3_BACKUP.$stamp"
$failed=Join-Path $reports "VERTEX_HUB_HOT_INSTALL_BROWSER_V3_FAILED.$stamp"
$report=Join-Path $reports "VERTEX_HUB_HOT_INSTALL_BROWSER_V3.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX — VERTEXHUB HOT INSTALL / PACKAGE BROWSER V3
...
'@
  ...
  # V3: robust line-based Cargo.toml dependency insertion.
  # V2 proved that regex insertion at the CR/LF boundary can corrupt CRLF.
  $tauriCargoText=[IO.File]::ReadAllText($tauriCargo)

  if($tauriCargoText -notmatch '(?m)^[ \t]*serde_json[ \t]*='){
    $eol=if($tauriCargoText.Contains("`r`n")){"`r`n"}else{"`n"}

    # Normalize only for deterministic line processing, then restore
    # the original EOL style when writing back.
    $normalized=$tauriCargoText.Replace("`r`n","`n").Replace("`r","`n")
    $lines=@($normalized -split "`n",-1)

    $dependencyIndex=-1
    for($index=0;$index -lt $lines.Count;$index++){
      if($lines[$index].Trim() -eq '[dependencies]'){
    ...
    if($dependencyIndex -lt 0){
      throw 'Tauri [dependencies] anchor missing; refusing dependency mutation.'
    }
    ...
    if($dependencyIndex -ge 0){
      $before=@($lines[0..$dependencyIndex])
    }

    if(($dependencyIndex+1) -lt $lines.Count){
      $after=@($lines[($dependencyIndex+1)..($lines.Count-1)])
    }

    $rebuilt=@()
    $rebuilt+=$before
    $rebuilt+='serde_json = "1"'
    $rebuilt+=$after

    $tauriCargoText=[string]::Join($eol,$rebuilt)
    WriteUtf8 $tauriCargo $tauriCargoText

    $verifyCargo=[IO.File]::ReadAllText($tauriCargo)
    if($verifyCargo -notmatch '(?m)^[ \t]*serde_json[ \t]*=[ \t]*"1"[ \t]*\r?$'){
      throw 'Tauri serde_json dependency write verification failed.'
    }

    # Guard against the exact V2 corruption class:
    # a carriage return not immediately followed by LF.
    if([regex]::IsMatch($verifyCargo,"`r(?!`n)")){
      throw 'Cargo.toml contains a bare carriage return after dependency mutation.'
    }

    Write-Host 'Tauri serde_json dependency   : ADDED' -ForegroundColor Green
    Write-Host 'Cargo.toml EOL integrity      : VERIFIED' -ForegroundColor Green
  }else{
    Write-Host 'Tauri serde_json dependency   : PRESENT' -ForegroundColor Green

    if([regex]::IsMatch($tauriCargoText,"`r(?!`n)")){
      throw 'Cargo.toml already contains a bare carriage return.'
    }
...
'@
  ...
  $audits=@(
    ...
    [pscustomobject]@{Name='FME bg-deep token';Pass=$browserNow.Contains('--vertex-bg-deep: #070b10')},
    [pscustomobject]@{Name='FME Vertex Blue token';Pass=$browserNow.Contains('--vertex-blue: #168cff')},
    [pscustomobject]@{Name='FME Bright Blue token';Pass=$browserNow.Contains('--vertex-blue-bright: #3ab8ff')},
    ...
    [pscustomobject]@{Name='Tauri serde_json dependency';Pass=([IO.File]::ReadAllText($tauriCargo) -match '(?m)^[ \t]*serde_json[ \t]*=')},
    [pscustomobject]@{Name='Cargo.toml EOL integrity';Pass=(-not [regex]::IsMatch([IO.File]::ReadAllText($tauriCargo),"`r(?!`n)"))},
  ...
  [ordered]@{
    schema='vertex.cic.vertex-hub-hot-install-package-browser.v3'
    timestamp=(Get-Date).ToString('o')
    status='GREEN'
    mission='VERTEXHUB HOT INSTALL / PACKAGE BROWSER'
    ui=[ordered]@{
      shell='FULLSCREEN_EQUIPMENT_DOCK'
      visual_language='FME_VERTEX_BLACK_NAVY_ELECTRIC_BLUE'
      search='ONLINE'
      filters=@('ALL','INSTALLED','ENABLED','AVAILABLE')
      trust_inspector='ONLINE'
      activity_mirror='ONLINE'
      responsive='ACTIVE'
      design_source='Vertex FM Engine / src/styles/main.scss'
      vertex_blue='#168cff'
      vertex_blue_bright='#3ab8ff'
      bg_deep='#070b10'
      bg_panel='#0c121a'
      bg_panel_raised='#111923'
    }
    runtime=[ordered]@{
      root=$runtimeRoot
      state=$runtimeState
    ...
    validation=[ordered]@{
      ...
      tauri_check='GREEN'
      cargo_toml_eol_integrity='VERIFIED'
  ...
  Write-Host @"
============================================================
 VERTEX — HOT INSTALL / PACKAGE BROWSER V3 GREEN
============================================================
 VertexHub Package Browser              ONLINE
 FME Vertex Equipment Dock UI           ONLINE
 Vertex Blue #168cff / #3ab8ff          LOCKED
 ...
 Tauri Check                            GREEN
 Cargo.toml EOL Integrity               VERIFIED
...
------------------------------------------------------------
 ...
 HUB UI:  FME-DERIVED VERTEX EQUIPMENT DOCK
 REPORT:  $report
------------------------------------------------------------
...
"@ -ForegroundColor Green
  ...
  Write-Host ' VERTEXHUB HOT INSTALL / PACKAGE BROWSER V3 RED — DAMAGE CONTROL' -ForegroundColor Red
