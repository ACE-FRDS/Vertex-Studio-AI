...
# ============================================================
# VERTEX - ADAPTIVE GRID EQUIPMENT WORKSPACE V7R2
#
...
$editorService=Join-Path $ui 'src\services\vertex-editor.ts'
...
$backup=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R2_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R2_FAILED.$stamp"
$report=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R2.$stamp.json"
...
function EnsureBootstrapBase {
  Write-Host "`n[BOOTSTRAP] TOPOLOGY DISCOVERY + runtime surface recovery" -ForegroundColor Yellow

  foreach($requiredBase in @(
    $cockpit,$shell,$editor,$editorService,$tauriCargo,$coreCargo,$tauriSrc
  )){
  ...
  if(-not(Test-Path -LiteralPath $developerShim)){
    ...
    if($developerSource){
      ...
      $relative=[IO.Path]::GetRelativePath($shimDir,$developerSource.FullName).Replace('\','/')
...
function unbound(): never {
  throw new Error(
    ...
    'V7R2 does not invent Tauri command names.'
  )
}
...
"@
  ...
  Write-Host "  Topology-aware bootstrap       : READY" -ForegroundColor Green
}
...
foreach($item in $required){
  if(-not(Test-Path -LiteralPath $item)){
    throw "Self-bootstrap artifact missing after recovery: $item"
  }
}
...
'@
  ...
  Write-Host "`n[6.5/10] ENSURE PRIMARY EDITOR WRAPPER" -ForegroundColor Yellow

  $editorCurrent=[IO.File]::ReadAllText($editor)

  if(-not $editorCurrent.Contains('VertexMainEditor')){
    $editorWrapper=@'
<script setup lang="ts">
...
'@
    ...
    Write-Host '  Primary Editor Wrapper         : CREATED' -ForegroundColor Green
  }else{
    Write-Host '  Primary Editor Wrapper         : PRESERVED' -ForegroundColor Green
  }
