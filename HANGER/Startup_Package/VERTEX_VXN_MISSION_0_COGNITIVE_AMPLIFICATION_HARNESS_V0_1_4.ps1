...
<#
VERTEX VXN — MISSION 0 COGNITIVE AMPLIFICATION HARNESS V0.1.4
...
Banner 'VERTEX VXN — MISSION 0 COGNITIVE AMPLIFICATION HARNESS V0.1.4'
...
foreach ($variant in $variants) {
    ...
    try {
        ...
        # Primary path: Hashtable gives schema-safe access.
        $parsedRaw = $candidate | ConvertFrom-Json -AsHashtable
        $jsonValid = $true

        if ($parsedRaw -is [System.Collections.IDictionary]) {
            $jsonShape = 'OBJECT'
            $parsedMap = $parsedRaw
        }
        elseif ($parsedRaw -is [System.Collections.IList] -and -not ($parsedRaw -is [string])) {
            $jsonShape = 'ARRAY'
            # If the model wrapped one object in an array, unwrap only for evaluation.
            if (@($parsedRaw).Count -eq 1 -and $parsedRaw[0] -is [System.Collections.IDictionary]) {
                $parsedMap = $parsedRaw[0]
                $jsonShape = 'ARRAY_SINGLE_OBJECT'
            }
        }
        elseif ($parsedRaw -is [string]) {
            $jsonShape = 'STRING'
        }
        else {
            $jsonShape = 'SCALAR'
        ...
        Write-Json (Join-Path $variantDir 'response.parsed.json') $parsedRaw
    }
    catch {
    ...
    Write-Host "      model=OK latency=$($call.LatencyMs)ms json=$jsonValid shape=$jsonShape schema=$([math]::Round($schemaScore,2)) lock=$lockScore scope=$scopeScore authority=$authorityScore missing=$($missingFields.Count) $evalState"
...
$receipt = [ordered]@{
    schema='vertex.vxn.mission0.receipt.v1.4'
    run_id=$runId
    timestamp=(Get-Date).ToString('o')
    provider=$runtime.Provider
    model=$runtime.Model
...
Banner 'VXN MISSION 0 V0.1.4 — FIRST SIGNAL COMPLETE'
Write-Host "Provider : $($runtime.Provider)"
Write-Host "Model    : $($runtime.Model)"
