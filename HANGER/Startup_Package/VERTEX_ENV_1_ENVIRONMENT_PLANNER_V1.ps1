...
function F($a,$s,$t,$d,$x='',$b=$false){[pscustomobject]@{area=$a;status=$s;title=$t;detail=$d;action=$x;blocking=$b}}
function Rank($s){switch($s){RED{4};ORANGE{3};YELLOW{2};UNKNOWN{1};default{0}}}
$roots=@(
...
Write-Host ' VERTEX ENV-1 - ENVIRONMENT PLANNER V1' -ForegroundColor Magenta
