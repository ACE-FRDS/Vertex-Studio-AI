...
impl VccFile {
    ...
    pub fn append(&self, r: &VccRecord) -> VsaResult<()> {
        let mut f = OpenOptions::new()
            ...
            .append(true)
            .open(&self.path)
            .map_err(|e| VsaError::Io(e.to_string()))?;
        let line = format!(
            "{}\t{}\t{}\t{}\n",
            clean(&r.stream_owner),
            clean(&r.record_type),
            clean(&r.status),
            clean(&r.text)
        );
    ...
    pub fn read_all(&self) -> VsaResult<Vec<VccRecord>> {
        ...
        Ok(s.lines()
            .filter_map(|l| {
                let p: Vec<_> = l.splitn(4, '\t').collect();
                if p.len() != 4 {
                    return None;
                }
                Some(VccRecord {
                    stream_owner: p[0].into(),
                    record_type: p[1].into(),
                    status: p[2].into(),
                    text: p[3].into(),
                })
            })
...
pub fn startup_order() -> [&'static str; 5] {
    [
        "StartupSource",
        "VCC",
        "VSP",
...
pub fn ensure_parent(path: &Path) -> VsaResult<()> {
    if let Some(p) = path.parent() {
        fs::create_dir_all(p).map_err(|e| VsaError::Io(e.to_string()))?
    }
    Ok(())
}
