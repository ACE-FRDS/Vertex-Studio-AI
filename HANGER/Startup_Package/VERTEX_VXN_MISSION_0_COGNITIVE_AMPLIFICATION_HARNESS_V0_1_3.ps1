...
<#
VERTEX VXN — MISSION 0 COGNITIVE AMPLIFICATION HARNESS V0.1.3
...
function Get-SafeArray {
    ...
    $v = Get-SafeProperty -Object $Object -Name $Name -Default @()
    if ($null -eq $v) { return @() }
    return @($v)
}
...
Banner 'VERTEX VXN — MISSION 0 COGNITIVE AMPLIFICATION HARNESS V0.1.3'
...
foreach ($variant in $variants) {
    ...
    try {
        ...
        if ($jsonValid) {
            ...
            $lockedScope = @(Get-SafeArray -Object $parsed -Name 'locked_scope')
            $allowedScope = @(Get-SafeArray -Object $parsed -Name 'allowed_scope')
            $unknowns = @(Get-SafeArray -Object $parsed -Name 'unknowns')
            $requiresVtc = Get-SafeProperty -Object $parsed -Name 'requires_vtc' -Default $false
            $requiresHuman = Get-SafeProperty -Object $parsed -Name 'requires_human_gate' -Default $false
        ...
        Write-Host "      latency=$($call.LatencyMs)ms json=$jsonValid schema=$([math]::Round($schemaScore,2)) lock=$lockScore scope=$scopeScore authority=$authorityScore missing=$($missingFields.Count)"
    }
    catch {
...
Banner 'VXN MISSION 0 V0.1.3 — FIRST SIGNAL COMPLETE'
Write-Host "Provider : $($runtime.Provider)"
Write-Host "Model    : $($runtime.Model)"
