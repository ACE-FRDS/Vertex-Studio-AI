...
pub enum MissionState { Pending, Ready, Running, Pass, Fail, Blocked, Unknown }
...
pub struct MissionGraph { pub units: HashMap<Id, WorkUnit> }

impl MissionGraph {
    pub fn add(&mut self, unit: WorkUnit) -> VsaResult<()> {
        if self.units.contains_key(&unit.id) { return Err(VsaError::Conflict(unit.id.0)); }
        self.units.insert(unit.id.clone(), unit);
        Ok(())
    }

    pub fn refresh_ready(&mut self) {
        let passed: HashSet<Id> = self.units.values()
            .filter(|u| u.state == MissionState::Pass)
            .map(|u| u.id.clone()).collect();
    ...
    pub fn ready_queue(&self) -> VecDeque<Id> {
        let mut ids: Vec<_> = self.units.values().filter(|u| u.state == MissionState::Ready).map(|u| u.id.clone()).collect();
        ids.sort();
        ids.into()
    }

    pub fn apply_footer(&mut self, id:&Id, footer:WorkerFooter) -> VsaResult<()> {
        let u = self.units.get_mut(id).ok_or_else(||VsaError::NotFound(id.0.clone()))?;
...
pub fn ultra_fine_decompose(title:&str, steps:&[&str]) -> Vec<WorkUnit> {
    ...
    steps.iter().enumerate().map(|(i,s)| WorkUnit{
        id:Id(format!("{}-{}", parent.0, i+1)),
        parent:Some(parent.clone()),
        title:format!("{title}: {s}"),
        depends_on: if i==0 {vec![]} else {vec![Id(format!("{}-{}",parent.0,i))]},
        state:MissionState::Pending,
        contract:ExecutionContract{role:"Local Developer".into(),scope:vec![],forbidden:vec![],stop_conditions:vec!["current reproducible blocker".into()]},
    }).collect()
}
