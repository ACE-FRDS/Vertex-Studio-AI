...
pub fn normalize(prompt:&HumanPrompt) -> CanonicalMission {
    ...
    let action=if lower.contains("作") || lower.contains("create") {"create"}
        else if lower.contains("修") || lower.contains("fix") {"repair"}
        else if lower.contains("調") || lower.contains("inspect") {"inspect"}
        else {"execute"};
    CanonicalMission{
        action:action.into(),
        object:prompt.original.clone(),
        constraints:vec!["preserve human intent".into(),"current evidence over assumption".into()],
...
mod tests {
    ...
    fn preserves_original() {
        let h=HumanPrompt{language:"ja".into(),original:"静的ホームページを作る".into()};
        let n=normalize(&h);
