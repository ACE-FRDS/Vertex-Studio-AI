...
# ============================================================
# VERTEX CIC — VERTEXHUB INSTALLER / LOADER V1
...
$backup=Join-Path $reports "VERTEX_HUB_INSTALLER_LOADER_V1_BACKUP.$stamp"
$failed=Join-Path $reports "VERTEX_HUB_INSTALLER_LOADER_V1_FAILED.$stamp"
$report=Join-Path $reports "VERTEX_HUB_INSTALLER_LOADER_V1.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX — VERTEXHUB INSTALLER / LOADER V1
...
if(-not $registryText.Contains('"schema": "vertex.hub.registry.v1"') -and
   -not $registryText.Contains('"schema":"vertex.hub.registry.v1"')){
  throw 'VertexHub registry v1 schema missing.'
}
if(-not $registryText.Contains($packageId)){throw "Package not registered: $packageId"}
if(-not $manifestText.Contains('"schema": "vertex.hub.package.v1"') -and
   -not $manifestText.Contains('"schema":"vertex.hub.package.v1"')){
  throw 'Hub package manifest v1 schema missing.'
}
...
'@
  ...
  if($manifest.package_id -ne $packageId -or $manifest.version -ne $packageVersion){
    throw "Manifest identity mismatch: $($manifest.package_id)@$($manifest.version)"
  }
...
'@
  ...
  foreach($audit in @(
    @('static component import',$catalogNow.Contains("import LiveFlightPanel from './packages/")),
    @('no arbitrary URL loader',-not $catalogNow.Contains('http://') -and -not $catalogNow.Contains('https://')),
    @('validated registry IPC',$runtimeNow.Contains("invoke<string>('vertex_hub_registry')")),
    @('runtime enable/disable',$dockNow.Contains('setHubPackageEnabled')),
    @('verified gate',$dockNow.Contains("registered(pkg) ? 'VERIFIED' : 'UNVERIFIED'")),
    @('controller mutation absent',-not $dockNow.Contains('FleetControllerSession'))
  )){
    if(-not $audit[1]){throw "Loader safety audit RED: $($audit[0])"}
    Write-Host ("  {0,-30} GREEN" -f $audit[0]) -ForegroundColor Green
  }

  Write-Host "`n[12/12] REPORT" -ForegroundColor Yellow

  [ordered]@{
    schema='vertex.cic.vertex-hub-installer-loader.v1'
  ...
  Write-Host @"
============================================================
 VERTEX — VERTEXHUB INSTALLER / LOADER V1 GREEN
============================================================
...
"@ -ForegroundColor Green
  ...
  Write-Host ' VERTEXHUB INSTALLER / LOADER V1 RED — DAMAGE CONTROL' -ForegroundColor Red
