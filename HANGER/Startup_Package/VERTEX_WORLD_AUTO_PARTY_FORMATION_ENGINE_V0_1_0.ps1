...
<#
VERTEX WORLD — AUTO PARTY FORMATION ENGINE V0.1.0
...
Banner 'VERTEX WORLD — AUTO PARTY FORMATION ENGINE V0.1.0'
...
foreach ($roleName in $weights.Keys) {
    ...
    $roleCandidates += [pscustomobject]@{
        ...
        Confidence=$roleEvidence.Tier
        Samples=$roleEvidence.Samples
...
$plan = [ordered]@{
    schema='vertex.world.ard.auto-party-formation.v1'
...
Write-Json $receiptPath ([ordered]@{
    schema='vertex.world.ard.auto-party-formation-receipt.v1'
...
foreach ($member in ($selected | Sort-Object FormationScore -Descending)) {
    ...
    Write-Host (
        "  {0}. {1,-14} model={2,-28} score={3} confidence={4}" -f `
        $position,
        $member.Role,
        $member.Model,
