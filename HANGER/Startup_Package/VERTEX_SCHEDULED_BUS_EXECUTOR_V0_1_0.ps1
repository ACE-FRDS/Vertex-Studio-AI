...
<#
VERTEX — SCHEDULED BUS EXECUTOR V0.1.0
...
Banner 'VERTEX — SCHEDULED BUS EXECUTOR V0.1.0'
...
foreach ($route in ($routes | Sort-Object { [int](Get-Prop $_ 'wave' 0) })) {
    ...
    while ($pending.Count -gt 0 -and $attempt -lt $MaxRetriesPerRoute) {
        ...
        if ($failed.Count -eq 0) {
            $lastStableWidth = $effectiveWidth
            continue
        }
...
Synthesize the field-lane outputs.
...
$receipt = [ordered]@{
    schema='vertex.world.ard.formation-execution.v1.1'
    run_id=$runId
