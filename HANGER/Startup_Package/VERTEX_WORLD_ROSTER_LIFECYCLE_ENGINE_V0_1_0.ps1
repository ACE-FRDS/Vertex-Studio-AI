...
<#
VERTEX WORLD — ROSTER LIFECYCLE ENGINE V0.1.0
...
Banner 'VERTEX WORLD — ROSTER LIFECYCLE ENGINE V0.1.0'
...
if ($null -ne $previousRoster) {
    $previousMembers = @(Get-Prop $previousRoster 'members' @())
}
...
foreach ($c in $characters) {
    ...
    $members += [ordered]@{
        character_id=[string](Get-Prop $c 'character_id' '')
...
$roster = [ordered]@{
    schema='vertex.world.rpg.roster-lifecycle.v1'
    ...
    policy=[ordered]@{
        note='Roster state is evidence-driven and provisional at low sample counts.'
...
Write-Json $receiptPath ([ordered]@{
    schema='vertex.world.rpg.roster-lifecycle-receipt.v1'
