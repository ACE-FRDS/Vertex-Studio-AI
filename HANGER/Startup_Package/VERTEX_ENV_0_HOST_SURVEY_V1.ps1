...
<#
VERTEX ENV-0
Vertex Host Survey & Environment Profile
...
function Find-App {
    ...
    @($Apps | Where-Object { $_.DisplayName -match $Pattern })
}
...
Write-Host ' VERTEX ENV-0 — HOST SURVEY & ENVIRONMENT PROFILE' -ForegroundColor Magenta
...
foreach ($key in $aiRuntime.Keys) {
    ...
    $found = $false
    if ($runtime.command -and $runtime.command.installed) { $found = $true }
    if ($runtime.apps -and @($runtime.apps).Count -gt 0) { $found = $true }
    Write-Host ("{0,-12}: {1}" -f $key, $(if ($found) { 'FOUND' } else { 'MISS' })) -ForegroundColor $(if ($found) { 'Green' } else { 'DarkGray' })
}
