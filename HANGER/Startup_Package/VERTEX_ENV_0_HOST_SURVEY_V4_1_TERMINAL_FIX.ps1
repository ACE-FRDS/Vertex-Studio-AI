...
<#
VERTEX ENV-0
Vertex Host Survey & Environment Profile V4.1
...
function Get-WindowsTerminalSettings {
    ...
    try {
        ...
        $defaultGuidProperty = $settings.PSObject.Properties['defaultProfile']
        $defaultGuid = if ($defaultGuidProperty) {
            [string]$defaultGuidProperty.Value
        } else {
            $null
        }

        $profilesProperty = $settings.PSObject.Properties['profiles']
        $profilesObject = if ($profilesProperty) { $profilesProperty.Value } else { $null }

        $listProperty = if ($profilesObject) {
            $profilesObject.PSObject.Properties['list']
        } else {
            $null
        }

        $profiles = if ($listProperty) { @($listProperty.Value) } else { @() }

        $defaultProfile = $profiles |
            Where-Object {
                $guidProperty = $_.PSObject.Properties['guid']
                $guidValue = if ($guidProperty) { [string]$guidProperty.Value } else { $null }
                $guidValue -eq $defaultGuid
            } |
            Select-Object -First 1
        ...
        if ($defaultProfile) {
            $nameProperty = $defaultProfile.PSObject.Properties['name']
            $sourceProperty = $defaultProfile.PSObject.Properties['source']
            $commandlineProperty = $defaultProfile.PSObject.Properties['commandline']
            $startingDirectoryProperty = $defaultProfile.PSObject.Properties['startingDirectory']

            if ($nameProperty) { $name = [string]$nameProperty.Value }
            if ($sourceProperty) { $source = [string]$sourceProperty.Value }
            if ($commandlineProperty) { $commandline = [string]$commandlineProperty.Value }
            if ($startingDirectoryProperty) { $startingDirectory = [string]$startingDirectoryProperty.Value }
        }
...
Write-Host ' VERTEX ENV-0 — HOST SURVEY & ENVIRONMENT PROFILE V4.1' -ForegroundColor Magenta
...
if ($terminalSettings.found) {
    ...
    Write-Host ("Default GUID      : {0}" -f $terminalSettings.default_profile_guid) -ForegroundColor Green
    Write-Host ("Default profile   : {0}" -f $terminalSettings.default_profile_name) -ForegroundColor Green
    Write-Host ("Profile source    : {0}" -f $terminalSettings.default_profile_source) -ForegroundColor Green
