...
param(
    ...
    [int]$MaxTokens = 1024,
    [int]$TimeoutSec = 180,
    [ValidateSet('UI_LOCK_SCOPE','TRANSACTION_SAFETY','MEMORY_RECALL','GENERAL')]
...
function Select-Model {
    ...
    $patterns = switch ($Tier) {
        '3B4B' { @(
            '(?i)(^|[^0-9])3[._-]?8b([^0-9]|$)',
            '(?i)(^|[^0-9])4b([^0-9]|$)',
            '(?i)(^|[^0-9])3b([^0-9]|$)'
        ) }
        '8B' { @('(?i)(^|[^0-9])8b([^0-9]|$)', '(?i)(^|[^0-9])7b([^0-9]|$)') }
        '12B' { @('(?i)(^|[^0-9])12b([^0-9]|$)', '(?i)(^|[^0-9])14b([^0-9]|$)') }
        '30B' { @(
            '(?i)(^|[^0-9])30b([^0-9]|$)',
            '(?i)(^|[^0-9])32b([^0-9]|$)',
            '(?i)(^|[^0-9])34b([^0-9]|$)'
    ...
    foreach ($p in $patterns) {
        $hit = @($Models | Where-Object { $_ -match $p } | Select-Object -First 1)
...
function Evaluate-Response {
    ...
    if ($Normalized.Success -and $null -ne $map) {
        foreach ($f in $required) {
            $exists = $false
            if ($map -is [System.Collections.IDictionary]) {
                if ($map.Contains($f)) { $exists = $true }
                else {
                    try { if ($map.ContainsKey($f)) { $exists = $true } } catch {}
                }
            }
        ...
        foreach ($f in $required) { $missing.Add($f) }
    }
    ...
    [pscustomobject]@{
        ...
        Lock=if ($locked.Count -gt 0) {1.0} else {0.0}
        Scope=if ($allowed.Count -gt 0) {1.0} else {0.0}
        Authority=(if ($vtc -eq $true) {0.5} else {0.0}) + (if ($human -eq $true) {0.5} else {0.0})
        Unknowns=if ($unknowns.Count -gt 0) {1.0} else {0.0}
...
function Diagnose {
    ...
    if ($MissionClass -eq 'MEMORY_RECALL' -and -not ($Toolbox -contains 'VCC_VSP')) {
        $d.Add('MEMORY_STARVED')
    }

    if ($MissionClass -eq 'TRANSACTION_SAFETY' -and $Eval.Authority -lt 1.0) {
        $d.Add('HIGH_RISK')
    }

    if ($null -ne $Call.PromptTokens -and [double]$Call.PromptTokens -gt 3000 -and $Eval.Schema -lt 1.0) {
        $d.Add('PROMPT_BLOAT')
    }

    if ($RetryCount -ge 2 -and ($Eval.Schema -lt 1.0 -or $Eval.Lock -lt 1.0 -or $Eval.Scope -lt 1.0)) {
        $d.Add('MODEL_STARVED')
    }
    ...
    foreach ($d in $Diagnosis) {
        switch ($d) {
            ...
            'MEMORY_STARVED' {
                ...
                $nextChannels.memory = [math]::Min(100, $nextChannels.memory + 25)
                $reasons.Add('Expand memory boundary.')
            }
            ...
            'HIGH_RISK' {
                ...
                $reasons.Add('Attach CANDIDATE_VTC and reduce authority throttle.')
            }
            ...
            'MODEL_STARVED' {
                ...
                if ($idx -ge 0 -and $idx -lt ($order.Count - 1)) {
                    ...
                    $reasons.Add("Escalate model tier: $Tier -> $nextTier")
                }
            }
...
Banner 'VERTEX BOOSTER — LIVE THROTTLE EXPERIMENT V0.1.0'
...
foreach ($tier in @('3B4B','8B','12B','30B')) {
    Write-Host "  $tier : $($modelByTier[$tier])"
}
...
for ($cycle = 1; $cycle -le $MaxCycles; $cycle++) {
    ...
    try {
        ...
        $diagnosis = @(Diagnose -Eval $eval -Call $call -MissionClass $MissionClass -Toolbox $toolbox -RetryCount $retryCount)
        ...
        if (-not $green) {
            ...
            Write-Host '      Decision : GREEN — hold runtime and stop.' -ForegroundColor Green
        }
        ...
        $order = @('3B4B','8B','12B','30B')
        $idx = [array]::IndexOf($order, $tier)

        if ($idx -ge 0 -and $idx -lt ($order.Count - 1)) {
            $candidateTier = $order[$idx + 1]
...
$receipt = [ordered]@{
    schema='vertex.booster.live-throttle-experiment.v1'
    run_id=$runId
