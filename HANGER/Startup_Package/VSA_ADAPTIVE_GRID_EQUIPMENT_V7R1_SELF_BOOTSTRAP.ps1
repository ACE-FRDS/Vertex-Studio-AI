...
# ============================================================
# VERTEX - ADAPTIVE GRID EQUIPMENT WORKSPACE V7R1
#
...
$developerService=Join-Path $ui 'src\services\developer.ts'
...
$backup=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R1_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R1_FAILED.$stamp"
$report=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R1.$stamp.json"
...
function EnsureBootstrapBase {
  Write-Host "`n[BOOTSTRAP] V6-equivalent runtime surface inspection" -ForegroundColor Yellow

  foreach($requiredBase in @(
    $cockpit,$shell,$editor,$developerService,$editorService,$tauriCargo,$coreCargo,$tauriSrc
  )){
...
"@
  ...
  Write-Host "  Self-bootstrap surface         : READY" -ForegroundColor Green
}
