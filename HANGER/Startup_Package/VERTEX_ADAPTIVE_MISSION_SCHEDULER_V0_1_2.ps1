...
<#
VERTEX — ADAPTIVE MISSION SCHEDULER V0.1.2
...
Banner 'VERTEX — ADAPTIVE MISSION SCHEDULER V0.1.2'
...
$formationCommandModel = [string](Get-Prop $command 'integrator_model' '')
$formationCommandScore = [double](Get-Prop $command 'command_assignment_score' 0)
$formationCommandTier = [string](Get-Prop $command 'staff_tier' 'UNCLASSIFIED')
$formationCommandEvidence = [double](Get-Prop $command 'command_evidence_score' 0)
...
# Command Specialty is advisory/fallback only.
$specialty = Get-CommandSpecialty `
    -SpecialtyIndex $commandSpecialty `
    -MissionClass $missionClass `
    -DefaultModel $formationCommandModel

$commandModel = $formationCommandModel
$commandSource = 'FORMATION_ASSIGNMENT'

if ([string]::IsNullOrWhiteSpace($commandModel)) {
    $commandModel = $specialty.Model
    $commandSource = 'COMMAND_SPECIALTY_FALLBACK'
}

$specialtyAligned = (
    -not [string]::IsNullOrWhiteSpace($specialty.Model) -and
    $specialty.Model -eq $commandModel
)
...
Write-Host "Command Source : $commandSource"
...
$queue = [ordered]@{
    schema='vertex.scheduler.mission-queue.v1.2'
...
$commandPhase = [ordered]@{
    ...
    formation_assignment=[ordered]@{
        model=$formationCommandModel
        staff_tier=$formationCommandTier
        command_evidence_score=$formationCommandEvidence
        command_assignment_score=$formationCommandScore
    }

    specialty_advisory=[ordered]@{
        model=$specialty.Model
        specialty_score=$specialty.SpecialtyScore
        confidence=$specialty.Confidence
        aligned_with_formation=$specialtyAligned
    }
...
$plan = [ordered]@{
    schema='vertex.scheduler.adaptive-mission-plan.v1.2'
    schedule_id=$scheduleId
    formation_id=$formationId
    mission_class=$missionClass
    ...
    scheduler=[ordered]@{
        ...
        command_handoff_green=($formationCommandModel -eq $commandModel)
...
$busHandoff = [ordered]@{
    schema='vertex.load-balancing-bus.handoff.v1.2'
    schedule_id=$scheduleId
    mission_class=$missionClass
...
Write-Json $receiptPath ([ordered]@{
    schema='vertex.scheduler.plan-receipt.v1.2'
    ...
    priority_ordering_green=$priorityOrderingGreen
    highest_priority_role=$highestPriorityRole
    formation_command_model=$formationCommandModel
    scheduler_command_model=$commandModel
    command_handoff_green=($formationCommandModel -eq $commandModel)
    command_source=$commandSource
...
Write-Host "  Source            : $commandSource"
Write-Host "  Formation Tier    : $formationCommandTier"
Write-Host "  Command Evidence  : $formationCommandEvidence"
Write-Host "  Command Score     : $formationCommandScore"
Write-Host "  Specialty Model   : $($specialty.Model)"
Write-Host "  Specialty Score   : $($specialty.SpecialtyScore)"
Write-Host "  Specialty Align   : $specialtyAligned"
...
Write-Host ''
Write-Host '[COMMAND HANDOFF VALIDATION]' -ForegroundColor Cyan
Write-Host "  Formation Command : $formationCommandModel"
Write-Host "  Scheduler Command : $commandModel"
Write-Host "  Handoff Match     : $($formationCommandModel -eq $commandModel)"
Write-Host "  Specialty Advisory: $($specialty.Model)"
