...
# ============================================================
# VERTEX CIC - VSA DOCKING COCKPIT V3R1
#
# Mission:
...
$tauriCheckTarget=Join-Path $startup '_build\VSA_TAURI_DOCKING_V3R1_CHECK'

$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backup=Join-Path $reports "VSA_DOCKING_COCKPIT_V3R1_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_DOCKING_COCKPIT_V3R1_FAILED.$stamp"
$report=Join-Path $reports "VSA_DOCKING_COCKPIT_V3R1.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX - VSA DOCKING COCKPIT V3R1
...
'@
  ...
  try{
    # IMPORTANT:
    # Never reuse the GUI dev target here. The preserved/moved dev cache can
    # contain absolute paths to the former src-tauri\target tree inside Tauri
    # generated permission metadata. Verification therefore gets a clean room.
    ...
    $checkCanonical=[IO.Path]::GetFullPath($tauriCheckTarget).TrimEnd('\')
    $devCanonical=[IO.Path]::GetFullPath((Join-Path $startup '_build\VSA_TAURI_DEV')).TrimEnd('\')

    if($checkCanonical.StartsWith($uiCanonical,[StringComparison]::OrdinalIgnoreCase)){
      throw "Tauri clean-room target is inside Vite UI root: $checkCanonical"
    }
    if($checkCanonical.Equals($devCanonical,[StringComparison]::OrdinalIgnoreCase)){
      throw 'Tauri check target must not reuse VSA_TAURI_DEV.'
    }

    Write-Host "  GUI dev target                  : PRESERVED / NOT USED" -ForegroundColor Green
    Write-Host "  Tauri check target              : $tauriCheckTarget" -ForegroundColor Green
    Write-Host "  Check cache                     : FRESH / CLEAN ROOM" -ForegroundColor Green
  ...
  [ordered]@{
    ...
    status='GREEN'
    mission='VSA DOCKING COCKPIT V3R1'
    display=[ordered]@{
      shell_grid='HEADER / LEFT DOCK / MAIN / RIGHT DOCK / FOOTER'
    ...
    validation=[ordered]@{
      ...
      tauri_check='GREEN'
      tauri_check_target='FRESH_ISOLATED_CLEAN_ROOM'
      gui_dev_target_reused='NO'
...
============================================================
 ...
 Tauri Check                              GREEN
 Tauri Check Cache                        FRESH / CLEAN ROOM
 GUI Dev Cargo Cache                      PRESERVED / NOT USED
...
"@ -ForegroundColor Green
  ...
  Write-Host ' VSA DOCKING COCKPIT V3R1 RED - DAMAGE CONTROL' -ForegroundColor Red
