...
<#
VERTEX ENV-2 V2.4.13 — PRODUCT LINEAGE BOUNDARY GATE
...
Write-Host ' VERTEX ENV-2 V2.4.13 — PRODUCT LINEAGE BOUNDARY GATE' -ForegroundColor Magenta
...
foreach ($c in $candidates) {
    ...
    $best = Get-SafeProperty -Object $c -Name 'best_replacement' -Default $null
    $identityDecision = [string](Get-SafeProperty -Object $c -Name 'identity_decision' -Default '')
    $revisedDecision  = [string](Get-SafeProperty -Object $c -Name 'revised_decision' -Default '')
    $versionAssessment = [string](Get-SafeProperty -Object $c -Name 'version_assessment' -Default 'UNKNOWN')

    $oldPath = [string](Get-SafeProperty -Object $c -Name 'old_program' -Default '')
    $newPath = if ($best) { [string]$best.path } else { '' }

    $oldRole = Get-ExecutableRole -Path $oldPath -DisplayName ([string](Get-SafeProperty -Object $c -Name 'display_name' -Default ''))
    $newRole = Get-ExecutableRole -Path $newPath -DisplayName ([string](Get-SafeProperty -Object $c -Name 'display_name' -Default ''))
    ...
    $results.Add([pscustomobject][ordered]@{
        ...
        reasons               = @($reasons)
        guards                = @($guards)
        firewall_rule_names   = @(Get-SafeProperty -Object $c -Name 'firewall_rule_names' -Default @())
        mutation              = 'NONE'
    })
...
$report = [ordered]@{
    ...
    mission       = 'VERTEX_ENV_2_V2_4_13_PRODUCT_LINEAGE_BOUNDARY_GATE'
...
Write-Host ' V2.4.13 PRODUCT LINEAGE GATE : GREEN' -ForegroundColor Green
