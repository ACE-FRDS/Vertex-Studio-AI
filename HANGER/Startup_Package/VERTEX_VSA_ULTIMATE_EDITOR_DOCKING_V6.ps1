...
$backup=Join-Path $reports "VSA_EDITOR_V6_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_EDITOR_V6_FAILED.$stamp"
$report=Join-Path $reports "VSA_EDITOR_V6.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX CIC - VSA ULTIMATE EDITOR DOCKING V6
...
try{
  ...
  Push-Location $ui
  try{
    if(-not $deps.ContainsKey('monaco-editor')){PmInstall 'monaco-editor@0.55.1' $false}
...
"@ -ForegroundColor Green
  ...
  Write-Host "`nV6 RED - DAMAGE CONTROL" -ForegroundColor Red
