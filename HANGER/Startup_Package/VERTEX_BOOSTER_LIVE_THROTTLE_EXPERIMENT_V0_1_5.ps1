...
function Get-ReliabilityScore {
    ...
    $schema = [math]::Max(0.0, [math]::Min(1.0, [double]$Eval.Schema))
    $lock = [math]::Max(0.0, [math]::Min(1.0, [double]$Eval.Lock))
    $scope = [math]::Max(0.0, [math]::Min(1.0, [double]$Eval.Scope))
    $authority = [math]::Max(0.0, [math]::Min(1.0, [double]$Eval.Authority))
    $confidence = [math]::Max(0.0, [math]::Min(1.0, [double]$Eval.Confidence))
    ...
    $efficiency = 1.0
    if ($null -ne $Call.PromptTokens) {
        $pt = [double]$Call.PromptTokens
        if ($pt -gt 3000) { $efficiency = 0.55 }
        elseif ($pt -gt 2000) { $efficiency = 0.70 }
        elseif ($pt -gt 1000) { $efficiency = 0.85 }
    }

    # Mission-specific hard gates remain stronger than the soft score.
    $hardGate = $true
    $hardReasons = New-Object System.Collections.Generic.List[string]

    if (-not $Eval.JsonValid -or $schema -lt 1.0) {
        $hardGate = $false
        $hardReasons.Add('SCHEMA_NOT_GREEN')
    }

    if ($lock -lt 1.0) {
        $hardGate = $false
        $hardReasons.Add('LOCK_NOT_GREEN')
    }

    if ($scope -lt 1.0) {
        $hardGate = $false
        $hardReasons.Add('SCOPE_NOT_GREEN')
    }

    if ($MissionClass -in @('TRANSACTION_SAFETY','MODEL_ESCALATION_TEST')) {
        if ($authority -lt 1.0) {
            $hardGate = $false
            $hardReasons.Add('AUTHORITY_NOT_GREEN')
        }

        if (-not ($Toolbox -contains 'CANDIDATE_VTC')) {
            $hardGate = $false
            $hardReasons.Add('CANDIDATE_VTC_MISSING')
        }
    }

    if ($MissionClass -in @('MEMORY_RECALL','MODEL_ESCALATION_TEST')) {
        if (-not ($Toolbox -contains 'VCC_VSP')) {
            $hardGate = $false
            $hardReasons.Add('VCC_VSP_MISSING')
        }
    }
    ...
    if ($MissionClass -eq 'MODEL_ESCALATION_TEST') {
        if (-not ($Toolbox -contains 'LOCK_SCOPE')) {
            $hardGate = $false
            $hardReasons.Add('LOCK_SCOPE_TOOL_MISSING')
        }
    }
    ...
    return [pscustomobject]@{
        ...
        Green=($hardGate -and $score -ge $threshold)
        Components=[ordered]@{
            schema=$schema
            lock=$lock
            scope=$scope
...
Banner 'VERTEX BOOSTER — LIVE THROTTLE EXPERIMENT V0.1.5'
...
for ($cycle = 1; $cycle -le $MaxCycles; $cycle++) {
    ...
    try {
        ...
        # Reliability has final authority over stopping. If hard gates pass and
        # composite reliability is green, remove MODEL_STARVED from the diagnosis.
        $reliabilityPreview = Get-ReliabilityScore `
            -Eval $eval `
            -Call $call `
            -Tier $tier `
        ...
        if ($reliabilityPreview.Green -and ($diagnosis -contains 'MODEL_STARVED')) {
            $diagnosis = @($diagnosis | Where-Object { $_ -ne 'MODEL_STARVED' })
            if ($diagnosis.Count -eq 0) {
                $diagnosis = @('HEALTHY')
            }
        }

        $reliability = Get-ReliabilityScore `
            -Eval $eval `
            -Call $call `
            -Tier $tier `
            -MissionClass $MissionClass `
        ...
        Write-Host "      Reliability : score=$($reliability.Score) threshold=$($reliability.Threshold) hardGate=$($reliability.HardGate)"
        if ($reliability.HardReasons.Count -gt 0) {
            Write-Host "      Hard Gates  : $($reliability.HardReasons -join ', ')"
        }
        ...
        $cycleRecord = [ordered]@{
            ...
            reliability=[ordered]@{
                ...
                hard_gate=$reliability.HardGate
                hard_reasons=@($reliability.HardReasons)
...
$receipt = [ordered]@{
    schema='vertex.booster.live-throttle-experiment.v1.5'
    run_id=$runId
