...
# ============================================================
# VERTEX - ADAPTIVE GRID EQUIPMENT WORKSPACE V7R10
#
...
$backup=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R10_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R10_FAILED.$stamp"
$report=Join-Path $reports "VSA_ADAPTIVE_GRID_V7R10.$stamp.json"
...
============================================================
 ...
 PS5.1 COMPAT / VERTEXHUB PRESERVE
============================================================
...
'@
  ...
  Write-Host ("  VertexHub topology              : {0}" -f (
    if($hubExpected){'DETECTED'}else{'NOT PRESENT'}
  )) -ForegroundColor $(if($hubExpected){'Green'}else{'Yellow'})

  Write-Host ("  GUI Preview topology            : {0}" -f (
    if($previewExpected){'DETECTED'}else{'NOT PRESENT'}
  )) -ForegroundColor $(if($previewExpected){'Green'}else{'Yellow'})
...
'@
  ...
  Write-Host ("  VertexHub preservation         : {0}" -f (
    if($hubExpected){'WIRED'}else{'N/A'}
  )) -ForegroundColor Green
  Write-Host ("  GUI Preview preservation       : {0}" -f (
    if($previewExpected){'WIRED'}else{'N/A'}
  )) -ForegroundColor Green
