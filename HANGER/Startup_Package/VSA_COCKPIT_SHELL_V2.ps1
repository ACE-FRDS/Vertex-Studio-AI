...
# ============================================================
# VERTEX CIC - VSA COCKPIT SHELL V2
#
# Mission:
...
$backup=Join-Path $reports "VSA_COCKPIT_SHELL_V2_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_COCKPIT_SHELL_V2_FAILED.$stamp"
$report=Join-Path $reports "VSA_COCKPIT_SHELL_V2.$stamp.json"
...
'@
  ...
  $audits=@(
    ...
    [pscustomobject]@{
      Name='Telemetry error strings safe'
      Pass=(
        $cockpitText.Contains("'runtime: ' + String(error)") -and
        $cockpitText.Contains("'hub: ' + String(error)") -and
        $cockpitText.Contains("'live: ' + String(error)")
      )
    },
    [pscustomobject]@{
      Name='No eval'
  ...
  [ordered]@{
    schema='vertex.cic.vsa-cockpit-shell.v2'
    timestamp=(Get-Date).ToString('o')
    status='GREEN'
    mission='VSA COCKPIT SHELL V2'
    ...
    telemetry=[ordered]@{
      ...
      fake_metrics='DENIED'
      telemetry_error_strings='POWERSHELL_SAFE_CONCAT'
  ...
  Write-Host @"
============================================================
 VERTEX - VSA COCKPIT SHELL V2 GREEN
============================================================
 ...
 Telemetry String Generation              POWERSHELL SAFE
...
"@ -ForegroundColor Green
  ...
  Write-Host ' VSA COCKPIT SHELL V2 RED - DAMAGE CONTROL' -ForegroundColor Red
