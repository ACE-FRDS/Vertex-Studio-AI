...
$widthMemoryRoot = Join-Path $VxnRoot 'runtime\booster\parallelism'
$widthMemoryPath = Join-Path $widthMemoryRoot 'ARD_PARALLEL_WIDTH_MEMORY.json'
$null = New-Item -ItemType Directory -Path $widthMemoryRoot -Force
...
function Diagnose-Integrator {
    ...
    try { $contentChars = ([string]$Call.Content).Length } catch {}
    try { $reasoningChars = ([string]$Call.Reasoning).Length } catch {}

    if ([string]::IsNullOrWhiteSpace($Text)) {
        $issues.Add('EMPTY_INTEGRATOR_OUTPUT')
    }

    if ($null -eq $Map) {
        $issues.Add('JSON_UNRESOLVED')
    }

    if ($contentChars -eq 0 -and $reasoningChars -gt 0) {
        $issues.Add('REASONING_FALLBACK_REQUIRED')
    }

    if ([string]$Call.FinishReason -eq 'length') {
        $issues.Add('OUTPUT_TRUNCATED')
    }

    if ($null -ne $Call.PromptTokens -and [double]$Call.PromptTokens -gt 2500) {
        $issues.Add('INTEGRATOR_PROMPT_BLOAT')
    }

    if ($PartyBundle.Length -gt 12000) {
        $issues.Add('PARTY_BUNDLE_LARGE')
    }

    if ($issues.Count -eq 0) {
        $issues.Add('NO_TRANSPORT_OR_FORMAT_ISSUE_DETECTED')
    }
    ...
    return [pscustomobject]@{
        ...
        PartyBundleChars=$PartyBundle.Length
    }
}
...
Banner 'VERTEX ARD — ADAPTIVE PARALLEL WIDTH EXPERIMENT V0.1.1'
...
$savedWidth = $null
$widthMemory = Read-JsonSafe $widthMemoryPath

if ($AutoStableWidth -and $null -ne $widthMemory) {
    try {
        if ([string]$widthMemory.model -eq $small) {
            $savedWidth = [int]$widthMemory.last_proven_stable_width
        }
    }
    catch {}
}

$requestedInitialWidth = $InitialParallelWidth

if ($null -ne $savedWidth -and $savedWidth -ge $MinParallelWidth) {
    $InitialParallelWidth = [math]::Min($PartySize, $savedWidth)
    Write-Host ''
    Write-Host '[1.5/7] STABLE WIDTH MEMORY'
    Write-Host "  Previous Stable Width : $savedWidth"
    Write-Host "  Requested Width       : $requestedInitialWidth"
    Write-Host "  Start Width           : $InitialParallelWidth" -ForegroundColor Green
}
else {
    Write-Host ''
    Write-Host '[1.5/7] STABLE WIDTH MEMORY'
    Write-Host '  No compatible previous stable width found.'
}
...
$lastProvenStableWidth = $null
$currentProbeWidth = $currentWidth
$wave = 0
...
while ($pending.Count -gt 0) {
    ...
    while (-not $waveDone -and $attempt -lt $MaxWaveRetries) {
        ...
        if ($failures.Count -eq 0) {
            ...
            $lastProvenStableWidth = [math]::Max(
                $MinParallelWidth,
                [math]::Min($currentWidth, $results.Count)
            )

            # Keep stable width separate from probe width.
            if ($currentWidth -lt $requestedInitialWidth) {
                $currentProbeWidth = [math]::Min($requestedInitialWidth, $currentWidth + 1)
                Write-Host "      STABLE : proven width = $lastProvenStableWidth" -ForegroundColor Green
                Write-Host "      PROBE  : next probe width = $currentProbeWidth" -ForegroundColor Green
            }
            else {
                $currentProbeWidth = $currentWidth
                Write-Host "      STABLE : proven width = $lastProvenStableWidth" -ForegroundColor Green
    ...
    if ($pending.Count -gt 0) {
        $currentWidth = [math]::Max(
            $MinParallelWidth,
            [math]::Min($currentProbeWidth, $pending.Count)
        )
    }
}
...
$integratorDiagnosis = Diagnose-Integrator `
    -Call $integrationCall `
    -Text $integrationText `
    -Map $integrationMap `
    -PartyBundle $bundle

Write-Json (Join-Path $runRoot 'INTEGRATOR_DIAGNOSTICS.json') $integratorDiagnosis
...
Write-Host "  Diagnostics : $($integratorDiagnosis.Issues -join ', ')"
...
if ($null -eq $lastProvenStableWidth) {
    $lastProvenStableWidth = [math]::Max($MinParallelWidth, [math]::Min($currentWidth, $PartySize))
}

$widthMemoryReceipt = [ordered]@{
    schema='vertex.ard.parallel-width-memory.v1'
    updated_at=(Get-Date).ToString('o')
    model=$small
    logical_party_size=$PartySize
    requested_initial_width=$requestedInitialWidth
    last_proven_stable_width=$lastProvenStableWidth
    current_probe_width=$currentProbeWidth
    source_run=$runId
}

Write-Json $widthMemoryPath $widthMemoryReceipt
...
$receipt = [ordered]@{
    schema='vertex.ard.adaptive-parallel-width-experiment.v1.1'
    run_id=$runId
    completed_at=(Get-Date).ToString('o')
    status=$status

    logical_party_size=$PartySize
    requested_initial_parallel_width=$requestedInitialWidth
    start_parallel_width=$InitialParallelWidth
    last_proven_stable_width=$lastProvenStableWidth
    current_probe_width=$currentProbeWidth
    ...
    integrator=[ordered]@{
        ...
        diagnostics=$integratorDiagnosis
    }
...
Write-Host "Requested Width     : $requestedInitialWidth"
Write-Host "Start Width         : $InitialParallelWidth"
Write-Host "Stable Width        : $lastProvenStableWidth"
Write-Host "Probe Width         : $currentProbeWidth"
