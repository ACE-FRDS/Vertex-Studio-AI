...
<#
VERTEX ENV-2
Package Lifecycle / Deployment Executor V1.1
...
function Invoke-EnsureDirectory {
    ...
    if ($Execute -and -not $existsAfter) {
        throw "ensure_directory verification RED: destination was not created: $destination"
    }
...
function Invoke-CopyFile {
    ...
    $destinationHashAfter = if ($Execute -and $destExistsAfter) { Get-Sha256 $destination } else { $null }

    $Receipt.after = [ordered]@{
        destination_existed = $destExistsAfter
        destination_sha256 = $destinationHashAfter
    }

    if ($Execute) {
        if (-not $destExistsAfter) {
            throw "copy_file verification RED: destination missing after copy: $destination"
        }

        if ($Receipt.before.source_sha256 -ne $destinationHashAfter) {
            throw "copy_file verification RED: SHA256 mismatch: $destination"
        }
    }
...
function Invoke-CopyTree {
    ...
    $Receipt.after = [ordered]@{
        destination_existed = $treeExistsAfter
    }

    if ($Execute -and -not $treeExistsAfter) {
        throw "copy_tree verification RED: destination tree missing after copy: $destination"
    }
...
Write-Host ' VERTEX ENV-2 — PACKAGE LIFECYCLE / DEPLOYMENT EXECUTOR V1.1' -ForegroundColor Magenta
Write-Host ' HUMAN GATE / RECEIPT / ROLLBACK EVIDENCE' -ForegroundColor Magenta
Write-Host ' RECEIPT STATE / POST-EXECUTION VERIFICATION' -ForegroundColor Magenta
Write-Host '============================================================' -ForegroundColor Magenta
...
foreach ($op in $ops) {
    ...
    try {
        ...
        if ($opReceipt.status -notin @('DRY_RUN','EXECUTED')) {
            throw "Operation receipt state RED: expected DRY_RUN/EXECUTED, got '$($opReceipt.status)'"
        }
...
$expectedOperationStatus = if ($execute) { 'EXECUTED' } else { 'DRY_RUN' }
$badOperations = @(
    $receipt.operations |
    Where-Object { $_.status -ne $expectedOperationStatus }
)

if ($badOperations.Count -gt 0) {
    $receipt.status = 'RED'
    $receipt | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $receiptPath -Encoding utf8
    throw "Final receipt verification RED: $($badOperations.Count) operation(s) are not $expectedOperationStatus."
}
