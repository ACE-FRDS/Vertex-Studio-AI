...
# ============================================================
# VERTEX CIC — LIVE SESSION BUS / EDITOR FLIGHT PANEL V8
...
#   - current AgentMissionDispatch does not require mission_id
#   - mission set comes from FleetReadyWave; confirmed mapping comes from process result
...
$backup=Join-Path $reports "LIVE_SESSION_BUS_V8_BACKUP.$stamp"
$failed=Join-Path $reports "LIVE_SESSION_BUS_V8_FAILED.$stamp"
$report=Join-Path $reports "LIVE_SESSION_BUS_V8.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX — LIVE SESSION BUS / EDITOR FLIGHT PANEL V8
...
foreach($requiredField in @('execution_id','agent_id')){
...
if($assignmentFields -contains 'mission_id'){
  Write-Host 'mission_id is available directly on AgentMissionDispatch.' -ForegroundColor Green
}else{
  Write-Host 'mission_id is NOT a direct AgentMissionDispatch field.' -ForegroundColor Yellow
  Write-Host 'Wave ready_mission_ids will be shown as the scheduled Mission set; completed process_results provide confirmed mission_id.' -ForegroundColor Yellow
}
...
$ignitionMatch=[regex]::Match($loopText,'let\s+(?<name>[A-Za-z_][A-Za-z0-9_]*)\s*=\s*ignite_current_wave\s*\(')
if(-not $ignitionMatch.Success){throw 'Cannot discover ignite_current_wave local variable.'}
$ignitionVar=$ignitionMatch.Groups['name'].Value
...
Write-Host "Ignition variable    : $ignitionVar" -ForegroundColor Green
            ...
            let executions = dispatch.$assignmentField
                ...
                .map(|execution| {
                    format!(
                        ...
                        vertex_live_quote(&format!("{:?}", execution.agent_id))
                    )
                })
                .collect::<Vec<_>>()
                .join(",");
...
fn vertex_live_publish_postflight<E: std::fmt::Debug, C: std::fmt::Debug>(
    ...
    let processes = record
        ...
        .map(|process| {
            format!(
                "{{\"mission_id\":{},\"program\":{},\"capability\":{},\"exit_code\":{},\"success\":{},\"duration_ms\":{},\"stdout\":{},\"stderr\":{}}}",
                vertex_live_quote(&process.mission_id),
                vertex_live_quote(&process.program),
                vertex_live_quote(&process.capability),
                process
                    .exit_code
                ...
                process.success,
                process.duration_ms,
                vertex_live_quote(&process.stdout),
                vertex_live_quote(&process.stderr)
            )
        })
        .collect::<Vec<_>>()
        .join(",");
    ...
    let json = format!(
        "{{\"schema\":\"vertex.mothership.live-session.v1\",\"kind\":\"wave_completed\",\"timestamp_ms\":{},\"session\":{{\"session_id\":{},\"status\":{},\"completed_waves\":{}}},\"wave\":{{\"sequence\":{},\"wave_id\":{},\"missions\":{},\"resulting_status\":{}}},\"dispatch\":{{\"dispatch_id\":{},\"process_results\":[{}]}},\"genesis\":{{\"event_count\":{},\"events_debug\":{}}},\"vsp\":{{\"checkpoint_debug\":{}}}}}",
        now_ms,
        ...
        vertex_live_quote(&format!("{:#?}", genesis_events)),
...
"@
  ...
  $postCall="`n    vertex_live_publish_postflight(&$recordVar, &$ignitionVar.dock.events, &$recordVar.checkpoint);"
  $patched=$patched.Insert($afterRecord2,$postCall)
  $patched=$patched+$helpers
...
fn vertex_runtime_directory() -> Result<std::path::PathBuf, String> {
    Ok(core_root()?.join("_vertex_runtime"))
}
...
const executions = computed(
  () => latest.value?.dispatch?.executions
    ?? latest.value?.dispatch?.process_results
    ?? [],
)

const scheduledMissions = computed(
  () => latest.value?.dispatch?.mission_set
    ?? latest.value?.wave?.ready
    ?? latest.value?.wave?.missions
    ?? [],
)
  ...
  <section class="flight" :class="{ expanded }">
    ...
    <div v-if="expanded" class="detail">
      <section>
        ...
        <h4>Executions / Process Results</h4>
        <article
        ...
        >
          ...
          <small>
            {{
              execution.mission_id
                || ('execution_id' in execution
                    ? 'Mission mapping deferred until process result evidence.'
                    : '-')
            }}
          </small>
        </article>
      ...
      <section>
        <h4>Genesis</h4>
        <pre>{{ latest?.genesis?.events_debug || 'No completed-wave Genesis payload yet.' }}</pre>
      </section>
...
'@
  ...
  [ordered]@{
    schema='vertex.cic.live-session-bus-editor-flight-panel.v8'
    ...
    surfaces=@{
      ...
      wave='LIVE'
      dispatch='LIVE'
      execution='LIVE_EXECUTION_ID_AGENT_AND_COMPLETED_PROCESS_RESULT'
      mission_mapping='WAVE_MISSION_SET_THEN_CONFIRMED_PROCESS_RESULT'
  ...
  Write-Host @"
============================================================
 VERTEX — LIVE SESSION BUS V8 GREEN
============================================================
 ...
 Mission Set                             LIVE
 Execution                               LIVE
 Confirmed Mission Mapping               POST-PROCESS EVIDENCE
 Genesis                                 LIVE
 VSP Checkpoint                          LIVE
...
"@ -ForegroundColor Green
  ...
  Write-Host ' LIVE SESSION BUS V8 RED — DAMAGE CONTROL' -ForegroundColor Red
