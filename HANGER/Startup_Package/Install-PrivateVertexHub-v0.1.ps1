...
$module    = Join-Path $hub 'src\private_control.rs'
$cargo     = Join-Path $hub 'Cargo.toml'

$stamp     = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDir = "G:\Vertex_Project\Vertex_Studio_AI\Gateway\backups\private-vertexhub-v0.1-$stamp"

Write-Host ''
Write-Host '==================================================' -ForegroundColor Cyan
Write-Host ' PRIVATE VERTEXHUB v0.1 - SAFE PATCH CORE' -ForegroundColor Cyan
Write-Host '==================================================' -ForegroundColor Cyan
...
if (-not (Test-Path $cargo)) {
    throw "Hub Cargo.toml not found: $cargo"
}
if (Test-Path $module) {
    throw "private_control.rs already exists; refusing to overwrite: $module"
}

New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
Copy-Item $lib   (Join-Path $backupDir 'lib.rs') -Force
Copy-Item $cargo (Join-Path $backupDir 'Cargo.toml') -Force
...
fn validate_relative_policy(relative: &Path) -> Result<(), String> {
    ...
    let file_name = relative
        ...
        .ok_or_else(|| format!("Private VertexHub target has no UTF-8 file name: {}", relative.display()))?
        .to_ascii_lowercase();
    ...
    let allowed = matches!(
        extension.as_str(),
        "rs" | "toml" | "vue" | "ts" | "tsx" | "js" | "json" | "md" | "css" | "yaml"
            | "yml" | "vxn" | "txt"
    );
...
fn resolve_existing_target(workspace_root: &Path, relative_value: &str) -> Result<(PathBuf, PathBuf), String> {
    let relative = safe_relative(relative_value)?;
    validate_relative_policy(&relative)?;

    let canonical_root = fs::canonicalize(workspace_root)
        .map_err(|error| format!("cannot canonicalize workspace root {}: {error}", workspace_root.display()))?;
...
pub fn read_private_source_snapshot(
    ...
    let content = String::from_utf8(bytes.clone())
        .map_err(|_| format!("Private VertexHub source is not UTF-8: {}", target.display()))?;
...
pub fn stage_private_patch(
    ...
    let (relative, target) =
        resolve_existing_target(workspace_root, &request.relative_path)?;
...
pub fn apply_staged_private_patch(
    ...
    if let Some(parent) = backup_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|error| format!("cannot create backup directory {}: {error}", parent.display()))?;
    }
    ...
    let parent = target
        .parent()
        .ok_or_else(|| format!("Private VertexHub target has no parent: {}", target.display()))?;

    let file_name = target
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or_else(|| format!("Private VertexHub target has no UTF-8 file name: {}", target.display()))?;
    ...
    fs::write(&temp, replacement_bytes)
        .map_err(|error| format!("cannot write Private VertexHub temp file {}: {error}", temp.display()))?;
    ...
    if applied_hash != replacement_sha256 {
        ...
        return match restore {
            Ok(()) => Err("Private VertexHub post-apply integrity mismatch; source restored".into()),
            Err(restore_error) => Err(format!(
    ...
    if let Err(error) = append_private_audit(
        ...
        if current_after_apply == receipt.applied_sha256 {
            let restore_temp = parent.join(format!(".{file_name}.vertex-audit-rollback-{request_id}.tmp"));
    ...
    fs::remove_file(&staged_path)
        .map_err(|error| format!("patch applied but cannot remove staged request {}: {error}", staged_path.display()))?;
...
pub fn rollback_private_patch(
    ...
    let (relative, target) =
        resolve_existing_target(workspace_root, &receipt.relative_path)?;
...
mod tests {
    ...
    fn stage_apply_and_rollback_round_trip() {
        ...
        let preview =
            stage_private_patch(&workspace, &control, &request).expect("stage patch");
        ...
        let applied =
            read_private_source_snapshot(&workspace, "crates/demo/src/lib.rs").expect("read applied");
...
try {
    Set-Content -Path $module -Value $moduleText -Encoding utf8

    $libText = Get-Content -Path $lib -Raw

    if ($libText -notmatch '(?m)^\s*pub\s+mod\s+private_control\s*;') {
        Add-Content -Path $lib -Value "`r`npub mod private_control;" -Encoding utf8
    }
    ...
    Write-Host ' PRIVATE VERTEXHUB v0.1 CORE INSTALLED' -ForegroundColor Green
    Write-Host '==================================================' -ForegroundColor Green
    ...
    Write-Host '  READ SNAPSHOT     : READY'
    Write-Host '  SHA256 LOCK       : READY'
    Write-Host '  PATCH STAGE       : READY'
    Write-Host '  HUMAN GATE        : READY'
    Write-Host '  TRANSACTION APPLY : READY'
    Write-Host '  AUDIT NDJSON      : READY'
    Write-Host '  ROLLBACK          : READY'
    Write-Host '  DELETE            : DISABLED'
    Write-Host '  GIT PUSH          : DISABLED'
    Write-Host '  NETWORK TRANSPORT : NOT YET WIRED'
    Write-Host ''
    Write-Host 'Next: publish private_control.rs with Safe Peek for inspection.' -ForegroundColor Yellow
}
catch {
    Write-Host ''
    ...
    Copy-Item (Join-Path $backupDir 'Cargo.toml') $cargo -Force
