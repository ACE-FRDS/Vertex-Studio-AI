...
$backup=Join-Path $reports "VSA_EDITOR_V3_BACKUP.$stamp"
$failed=Join-Path $reports "VSA_EDITOR_V3_FAILED.$stamp"
$report=Join-Path $reports "VSA_EDITOR_V3.$stamp.json"
...
Write-Host @'
============================================================
 VERTEX CIC — VSA ULTIMATE EDITOR DOCKING V3
...
<template>
<button class="vx-launch" :class="{on:online}" @click="show"><i/>VSA EDITOR</button>
<Teleport to="body">
<section v-if="shown" class="vx">
<header><b>VX</b><div><strong>Vertex Ultimate Editor</strong><small>Command · Development · Inspector Surface</small></div><span class="grow"/><em :class="{ok:rt?.mothership}">Mothership {{rt?.mothership?'DETECTED':'OFFLINE'}}</em><em :class="{ok:rt?.real_hyper_agent}">Hyper Agent {{rt?.real_hyper_agent?'ABOARD':'N/A'}}</em><button @click="refresh">Refresh</button><button :disabled="!dirty||busy" @click="save">Save</button><button @click="shown=false">×</button></header>
...
"@ -ForegroundColor Green
  ...
  Write-Host "`nV3 RED — DAMAGE CONTROL" -ForegroundColor Red
  ...
  foreach($p in @($pkg,$app,$main)+$locks){if($p){R $p}}
